...
describe("GET /users/:username", function () {
  ...
  test("forbidded for non admin or not the same user", async () => {
    const resp = await request(app)
      .get(`/users/u2`)
      .set("authorization", `Bearer ${u1Token}`);
    expect(resp.statusCode).toEqual(403);
...
describe("PATCH /users/:username", () => {
  ...
  test("forbidden for not admin or different user", async function () {
    const resp = await request(app)
      .patch(`/users/u2`)
      .send({
        firstName: "New",
...
describe("DELETE /users/:username", function () {
  ...
  test("forbidden for not admin or not the same user", async function () {
    const resp = await request(app)
      .delete(`/users/u2`)
      .set("authorization", `Bearer ${u1Token}`);
    expect(resp.statusCode).toBe(403);
